...
model = AutoModelForSeq2SeqLM.from_pretrained("./t5-base").to("cuda") # Using CPU but if GPU add .to("cuda")
...
def preprocess(example):
    ...
    inputs = tokenizer(
        example["article"], max_length=512, padding="max_length", truncation=True, return_tensors="pt"
    )
...
tracker = CarbonTracker(epochs=num_epochs, components="gpu", monitor_epochs = -1, log_dir = "./carbon-results-t5")
...
for epoch in range(num_epochs):
    ...
    for batch in tqdm(dataloader):
        ...
        # Forward pass using mixed precision to reduce memory usage
        with autocast():
    ...
    with torch.no_grad():
        for batch in tqdm(val_dataloader):
            ...
            with autocast(): # mixed precision on validation loop
                outputs = model(input_ids=input_ids, attention_mask=attention_mask, labels=labels)
...
model.save_pretrained("./t5-base-trained")
tokenizer.save_pretrained("./t5-base-trained")
